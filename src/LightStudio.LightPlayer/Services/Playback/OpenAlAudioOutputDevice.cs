...
{
    // ALC_FREQUENCY attribute key (alc.h): the mixer rate OpenAL negotiated with the device.
    ...
    /// <summary>
    /// The mixer sample rate (Hz) OpenAL negotiated with the system output device when this
    /// device's context was created, or 0 when it cannot be read. OpenAL fixes this for the life
    /// of the context, so comparing it against a fresh system-rate query reveals when the user has
    /// switched to an output device that runs at a different rate.
    /// </summary>
    public int DeviceSampleRate
    {
        get
        {
            if (disposed || device == ALDevice.Null)
            {
                return 0;
            }
