...
namespace LightStudio.MediaLibraryCore.Tools
{
    public static class TrieTreeExtensions
    {
            ...
            IEqualityComparer<TKey> comparer)
        {
            ...
            {
                var keys = keySelector(value);
                if (keys == null)
                {
                    // Skip items without a key sequence (e.g. a media file with no title)
                    // so they don't break index construction.
                    continue;
                }

                tree.Insert(keys.GetEnumerator(), value);
            }
            return tree;
        }
    }
